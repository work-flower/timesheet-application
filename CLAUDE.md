...
- **Other:** `@uiw/react-md-editor` (markdown notes), `multer` + `sharp` (expense attachments + thumbnails), `@aws-sdk/client-s3` + `archiver` + `tar` + `node-cron` (R2 cloud backup), `jose` (Cloudflare Access JWT verification for the admin surface), `node-html-parser` (server-side pruning of Copilot page snapshots), `dotenv`
...
- **Current page content:** `PUT`/`DELETE /api/current-page-content` — the Copilot pane pushes/purges a snapshot of the page the user is viewing (raw `<main>` HTML; server converts it asynchronously to compact markdown via `node-html-parser`). Per-identity in-memory store (`pageContentStore`), no TTL, no GET route — read in-process by the `get_page_content` agent tool only. Never persisted, never backed up, excluded from payload logging. See `.claude/docs/agents.md` → Page context.
