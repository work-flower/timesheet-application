...
export async function confirm(id) {
  ...
  // Assign invoice number only if not already set (permanent once assigned)
  let invoiceNumber = invoice.invoiceNumber;
  if (!invoiceNumber) {
    const settingsDoc = await settings.findOne({});
    const currentSeed = settingsDoc?.invoiceNumberSeed || 0;
    const nextNumber = currentSeed + 1;
    invoiceNumber = `JBL${String(nextNumber).padStart(5, '0')}`;
  ...
  if (tsIds.length) {
    await timesheets.update(
      { _id: { $in: tsIds } },
      { $set: { invoiceId: id, isLocked: true, isLockedReason: `Locked by invoice ${invoiceNumber}` } },
      { multi: true }
    );
  }
  if (expIds.length) {
...
export async function post(id) {
  ...
  const now = new Date().toISOString();
  await invoices.update({ _id: id }, {
    $set: { status: 'posted', isLocked: true, isLockedReason: 'Posted invoice', updatedAt: now }
  });
  return getById(id);
}
...
export async function unconfirm(id) {
  ...
  if (tsIds.length) {
    await timesheets.update(
      { _id: { $in: tsIds } },
      { $unset: { invoiceId: true, isLocked: true, isLockedReason: true } },
      { multi: true }
    );
  }
  if (expIds.length) {
