...
> **Status:** Phases 1 ("it talks"), 2 ("it routes"), 3 ("it delegates") and the tools phase ("it acts": per-card tool grants, in-loop execution, action-card write confirmations, tools in the routing corpus) implemented. Beyond that (see `## Roadmap`): RAG Provider knowledge consumption, `.cred` secrets vault, declarative admin-registrable tools.
...
- `server/routes/mcp.js` — consumes the shared `agentToolRegistry.js` (`tools[]`/`handlers{}`); `tools/list` strips the agent-layer metadata (`kind`/`access`) so the MCP wire surface is byte-identical to pre-metadata builds.
...
- Registry changes: MCP `tools/list` must stay exactly `{name, description, inputSchema}` per tool (byte-diff it); the routing index hash covers tool descriptions, so description edits rebuild the index on next use.
...
| App tool registry (shared with MCP) | `server/services/agentToolRegistry.js` (`tools[]` + `handlers{}` + per-tool `kind`/`access` metadata + `toolsByName`/`canUseTool`; `routes/mcp.js` imports it and STRIPS the metadata on `tools/list`) |
...
24. **Uniform tool semantics at any depth** (master loop, @mention/auto-route takeover, `ask_agent` sub-loop): granted **read** tools execute immediately under the caller's ALS identity — the pipeline enforces roles/record-scoping/fls with zero extra code; handler errors become result text (`Tool error: …`) so the loop continues. Granted **write** tools (registry `kind: 'write'`) NEVER execute inline: the loop appends a `proposal` row + a pending tool_result stating the write did NOT run, emits an SSE `{type:'proposal'}` event, and continues normally (the next round narrates the card). Grants live in `manifest.json` `tools: [names]`; assembly (`resolveGrantedTools`) warns+skips stale names and pre-filters via `canUseTool` (registry `access: {table, op}` vs the caller's grants) so a caller is never offered a tool that is guaranteed to be denied.
...
A tool = a contract (`{name, description, inputSchema, kind, access}`) + a handler (async fn calling pipeline-wrapped services) — both in `server/services/agentToolRegistry.js`, the single source of truth. Behaviour is CODE: register/retire is a code change + server restart, never runtime data.

**Register**: add the def (with `kind: 'read'|'write'` and `access: {table, op}`) + handler to the registry. Everything downstream follows automatically: MCP `tools/list` (metadata stripped), the `/admin/api/agents/tools` endpoint → a new checkbox in every card designer, the routing corpus (description embeds — corpus hash changes, index rebuilds lazily), the eval-set tool dropdown, static/discover delivery. No card is affected until an admin grants it.

**Retire**: remove it from the registry. Downstream self-heals: card manifests still listing the name warn+skip at assembly (grant kept in case it returns); pending action cards for it resolve as `failed` with a clear message on Confirm; its corpus entry disappears on the next hash check; eval examples targeting it start showing as misroutes in Run Evals — the curation signal to delete or relabel them.
...
- **Declarative admin-registrable tools**: JSON tool cards (service/endpoint + arg mapping + schema) executed by one generic reviewed handler — registrable at runtime without arbitrary code; meshes with the dormant `skills/` folders (parked design conversation).
