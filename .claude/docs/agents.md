...
### Page context (current-page visibility)
| Layer | File |
|---|---|
| Per-user snapshot store (in-memory Map, NO TTL; entry lives until pane-close DELETE / overwrite / restart; never persisted, never backed up) | `server/services/pageContentStore.js` (also exports `identityKey()` — ALS effective identity, `'local'` in legacy mode — and `peek()` — pointer-only read stamped onto routing evidence) |
| Server-side async strip (`node-html-parser`; deterministic HTML→**markdown** emission — grids/`role=grid` → md tables, form controls → `[input: value]`/`[x]` markers, links kept, buttons as `[text]`; drops script/style/svg/aria-hidden; syntactic visibility only; plain-text fallback, never throws) | `server/services/pageContentStrip.js` |
| Push/purge route (`PUT` + `DELETE /api/current-page-content`, 204; 4MB cap → 413; **no GET** — reads are in-process only; excluded from `logPayloads` in `server/index.js`) | `server/routes/pageContent.js` |
| Tool handler + seed (`get_page_content`, kind read, `access: null` — table-less, canUseTool always passes) | `server/services/agentToolRegistry.js` |
| Client capture (clone `<main>` + form-control value sweep + `outerHTML`; no client-side pruning) | `app/src/utils/capturePageContent.js` |
| Publisher hook (mount/route-change push, unmount keepalive DELETE, `publishNow()` for send-time freshness) | `app/src/hooks/usePageContentPublisher.js` → wired in `CopilotPane.jsx` (`send()` awaits `publishNow()` before `streamChat`) |
| API client (raw fetch — 204 bodies, keepalive remove) | `app/src/api/index.js` → `pageContentApi` |
| Routing fallback tier (`fallbackAgentSlug`) | `server/services/routingConfigService.js` + `agentChatService.js` (`resolveFallbackCandidate`) + `admin/src/pages/agents/RoutingPage.jsx` |
...
29. **Page-snapshot identity comes from ALS ONLY — never from a URL segment, never from tool input.** The store key is `identityKey()` (effective ALS identity; impersonation keys to the target; `'local'` in legacy mode); the push/purge routes derive it from the request, and `get_page_content` has a deliberately EMPTY inputSchema — a `userId` parameter would turn a prompt injection into a cross-user page read. There is no HTTP GET on the store: reads happen exclusively in-process via the tool handler. The snapshot content itself is privilege-neutral (the browser only ever held what the caller's roles/fls allowed it to see). Lifecycle: the "service" runs exactly while the pane is mounted — push on mount/route-change/message-send (send-time push is awaited, making the sending tab win multi-tab last-write-wins), keepalive DELETE on unmount, NO TTL by design. The tool appears on the MCP surface too (shared cache) — harmless, MCP callers just get `page_view_unavailable`.
...
- **Controlled React inputs never serialise their live value — `outerHTML` shows the LOADED state, not what the user typed.** React keeps the current value in DOM properties; the `value` attribute holds the initial value. A naive `main.outerHTML` snapshot therefore silently loses unsaved edits — the single most valuable thing in a page snapshot. `capturePageContent` clones `<main>` and sweeps `input/textarea/select` explicitly (`.value`/`.checked`/selected option text → attributes on the clone, passwords masked) before serialising. If the snapshot ever grows scope, keep the sweep aligned with the capture root.
- **The page snapshot must never include the Copilot pane.** The pane renders OUTSIDE `<main>` (AppLayout sibling), which is what makes `<main>`-scoped capture safe — body-scoped capture would feed the conversation back into itself (page content containing the chat containing prior page content, recursively).
