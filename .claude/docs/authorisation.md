...
- **Read-model embeds persisted by `update()` echoes leaked past row-level security — confirmed in production.** Detail endpoints used to embed related records (client → projects/timesheets/expenses/invoices arrays; invoice → client doc + clientProjects + linkedTransactions; transaction → linked invoices/expenses); forms PUT the full response back and each service's `update()` stripped only 4 protected fields, so the embeds were stored onto the parent documents. A stored copy is invisible to the related table's row filter — the filter applies to rows of that table, never to fields of a parent document — so scoping silently didn't cover it. Fix: detail endpoints are now lean (no embeds), related data is opt-in via central `$expand` (`server/expand.js`) resolving through the wrapped collections — expanded docs are caller-scoped and fls-masked per their own table, and child-collection expands 403 fail-loud for callers lacking read on the expanded table (baseline reads cover parent expands only) — plus `update()` read-model strips and a `repair-derived-fields` data cleanup. This also removed the previously-hidden cross-table 403s that invoice/expense/transaction detail enrichment triggered for scoped roles.
